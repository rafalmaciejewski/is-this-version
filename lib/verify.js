...
async function verify(version) {
    try {
        ...
        const packageVersion = await parsePackageJson();
        const lockVersion = await parsePackageLock();

        if (
            packageVersion === expectedVersion &&
            lockVersion === expectedVersion
        ) {
            console.log(`${chalk.green('Yes')}, it is ${emoji.get('ok_hand')}`);
            process.exit(0);
        } else {
            console.error(`${chalk.red('No')}, it's not ${emoji.get('police_car')}`);
            console.log(`
